...
#    `functionName` is a function's name in your module (e.g., `comm_metricsNWTEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/AARAS2/Downloads/NWT/modules/comm_metricsNWT/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("comm_metricsNWT")
  path <- list(modulePath = "C:/Users/AARAS2/Downloads/NWT/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    comm_metricsNWT = list(.saveInitialTime = NA)
  )
  ...
  if (exists("comm_metricsNWTEvent1", envir = .GlobalEnv)) {
    simOutput <- comm_metricsNWTEvent1(mySim)
  } else {
  ...
  if (exists("comm_metricsNWTEvent2", envir = .GlobalEnv)) {
    simOutput <- comm_metricsNWTEvent2(mySim)
  } else {
